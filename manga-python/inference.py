...
def model2annotations(model_path, img_dir_list, save_dir, save_json=False, visual_mode=False, ocr=None):
    ...
    for img_path in tqdm(imglist):
        ...
        # cv2.imshow('rst', resize(img))
        # cv2.imshow('mask', resize(mask))
        # cv2.imshow('mask_refined', resize(mask_refined))
        # cv2.waitKey(0)
...
def to_ocr(ocr, img, blk_list:  List[TextBlock]):
    if ocr is None:
        ValueError("Can not OCR")

    for ii, blk in enumerate(blk_list):
        bx1, by1, bx2, by2 = blk.xyxy # 말풍선의 좌상 xy 우하 xy
        crop_img = crop_image(img, bx1, by1, bx2, by2)
        text = ocr(crop_img)
        print(ii, text)
...
if __name__ == '__main__':
    ...
    ocr = MangaOcr()
    model2annotations(model_path, img_dir, save_dir, save_json=True, ocr=ocr)
